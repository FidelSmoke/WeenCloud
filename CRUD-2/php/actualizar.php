...
if(!empty($_POST)){
	if(isset($_POST["Lutrabajo"]) &&isset($_POST["experiencia"]) &&isset($_POST["salario"]) &&isset($_POST["contactanos"]) &&isset($_POST["Cargo_Categoria"])){
		if($_POST["Lutrabajo"]!=""&& $_POST["experiencia"]!=""&&$_POST["contactanos"]!=""){
			...
			$query = $con->query($sql);
			if($query!=null){
				print "<script>alert(\"Actualizado exitosamente.\");window.location= 'php/formulario.php';</script>";
			}else{
				print "<script>alert(\"No se pudo actualizar.\");window.location= 'php/formulario.php';</script>";
